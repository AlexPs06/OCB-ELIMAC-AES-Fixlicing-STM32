...
extern void two_Rounds_aes128_encrypt_ffs(uint8_t* ctext, uint8_t* ctext_bis, const uint8_t* ptext, const uint8_t* ptext_bis, const uint32_t* rkey);
...
void OCB(unsigned char* plaintext, unsigned char* ciphertext, unsigned char* nonce, 
    ...
    aes128_keyschedule_ffs_lut(rkeys_ffs, key0);
    //aes128_2rounds_keyschedule_ffs(two_AES_keys_ffs, key_2AES);
    aes128_keyschedule_ffs_lut(two_AES_keys_ffs, key_2AES);
    ...
    for (size_t i = 0; i < size/2; i++){
        ...
        H(c_N_0, c_N_1, two_AES_keys_ffs, 2);
...
void PMAC( unsigned char* nonce, unsigned char* asociated_data, unsigned int asociated_data_size,unsigned char* key, unsigned char* tag){
    ...
    aes128_keyschedule_ffs_lut(rkeys_ffs, key);
    aes128_keyschedule_ffs_lut(two_AES_keys_ffs, key_2AES);
